...
//
// spec:
//   shape: "round"|"egg"|"pear"|"square"  体型
//   top/bottom/width: からだの範囲 (width=中心からの半幅)
//   eyeY: 目の上端の行 / eyes: "big"|"small"|"sleepy" / fierce: まゆ毛
//   mouth: "smile"|"open"|"w"|"fang"|"beak"|"none"
//   blush: ほっぺ / belly: おなかの色分け / feet, arms: 手足
//   parts: [[パーツ名, 引数...], ...]
//   pal: 色 (a=本体 h=ハイライト d=かげ b=おなか + パーツ用)
...
function triUp(g, cx, baseY, halfW, h, ch) { // 上向き三角 (cx中心)
  for (let i = 0; i < h; i++) {
...
function bodyWidths(spec) {
  ...
  for (let y = top; y <= bottom; y++) {
    ...
    let w;
    switch (spec.shape) {
      case "egg": w = maxW * Math.pow(1 - Math.pow(s, 2.4), 1 / 2.4) * (0.72 + 0.28 * t); break;
      case "pear": w = maxW * Math.pow(1 - Math.pow(s, 2.6), 1 / 2.6) * (0.58 + 0.42 * t); break;
...
// ---------- パーツ ----------
// 中央パーツは cx=15 (ミラーでつながる)、横パーツは左側に置く
const PARTS = {
  twinleaf(g, s) { // ふたばの芽
    const t = s.top;
    oval(g, 11, t - 5, 2, 1, "l"); set(g, 10, t - 6, "l"); set(g, 11, t - 6, "l");
    set(g, 13, t - 4, "l"); set(g, 14, t - 3, "l");
    fillRect(g, 15, t - 3, 15, t - 1, "l");
  },
  leafcap(g, s) { // 葉っぱのぼうし (フォレスタ)
    oval(g, 15, s.top + 1, s.width - 1, 4, "l");
    set(g, 9, s.top - 1, "p"); set(g, 13, s.top - 3, "y");
  },
  leaf1(g, s) { // 一枚葉
    oval(g, 15, s.top - 4, 2, 2, "l");
    set(g, 15, s.top - 1, "k"); set(g, 15, s.top - 2, "k");
  },
  flame(g, s) { // あたまのほのお
    triUp(g, 15, s.top - 1, 3, 6, "f");
    triUp(g, 15, s.top - 1, 1, 4, "g");
  },
  bolt(g, s) { // いなずま
    const t = s.top;
    set(g, 14, t - 6, "z"); set(g, 15, t - 6, "z");
    set(g, 13, t - 5, "z"); set(g, 14, t - 5, "z");
    set(g, 14, t - 4, "z"); set(g, 15, t - 4, "z");
    set(g, 14, t - 3, "z"); set(g, 15, t - 2, "z"); set(g, 15, t - 1, "z");
  },
  droplet(g, s) { // しずく
    set(g, 15, s.top - 6, "h");
    oval(g, 15, s.top - 3, 1, 2, "h");
  },
  drip(g, s) { // ぷにっとしたたれ
    oval(g, 15, s.top - 3, 1, 1, "a");
    fillRect(g, 15, s.top - 2, 15, s.top, "a");
  },
  earCat(g, s) { // ねこみみ
    triUp(g, 6, s.top + 1, 2, 5, "a");
    triUp(g, 6, s.top, 1, 3, "p");
  },
  earRound(g, s) { // くまみみ
    oval(g, 6, s.top - 1, 2, 2, "a");
    oval(g, 6, s.top - 1, 1, 1, "p");
  },
  earBunny(g, s) { // うさみみ
    oval(g, 7, s.top - 5, 1, 5, "a");
    oval(g, 7, s.top - 5, 0, 3, "p");
  },
  hornNubs(g, s) { // ちいさなつの
    triUp(g, 8, s.top, 1, 3, "y");
  },
  hornLong(g, s) { // りっぱなつの
    triUp(g, 7, s.top, 1, 3, "y");
    fillRect(g, 5, s.top - 5, 6, s.top - 2, "y");
    set(g, 4, s.top - 6, "y"); set(g, 5, s.top - 6, "y");
  },
  antler(g, s) { // えだづの (でんせつ)
    fillRect(g, 6, s.top - 6, 7, s.top - 1, "g");
    fillRect(g, 4, s.top - 5, 5, s.top - 4, "g");
    set(g, 4, s.top - 6, "g");
    set(g, 8, s.top - 4, "g"); set(g, 9, s.top - 5, "g");
  },
  antennaBalls(g, s) { // まるアンテナ
    set(g, 10, s.top - 2, "k"); set(g, 9, s.top - 3, "k");
    oval(g, 9, s.top - 5, 1, 1, "z");
  },
  finGills(g, s) { // みずかきエラ
    const y = s.eyeY + 1;
    oval(g, 3, y, 1, 2, "g");
  ...
  flowerTop(g, s) { // あたまのはな
    const t = s.top;
    set(g, 15, t - 4, "y"); set(g, 14, t - 4, "y");
    oval(g, 13, t - 4, 1, 1, "p"); oval(g, 15, t - 6, 1, 1, "p"); oval(g, 15, t - 2, 1, 1, "p");
  },
  flowerCrown(g, s) { // はなかんむり
    oval(g, 7, s.top, 1, 1, "p");
    oval(g, 11, s.top - 2, 1, 1, "p");
    oval(g, 15, s.top - 3, 1, 1, "y");
  },
  rockCrown(g, s) { // いわのでこぼこ
    oval(g, 6, s.top, 2, 2, "a");
    oval(g, 11, s.top - 1, 2, 2, "a");
    oval(g, 15, s.top - 2, 2, 2, "a");
  },
  crownGold(g, s) { // おうさまのかんむり
    fillRect(g, 10, s.top - 2, 15, s.top, "y");
    set(g, 10, s.top - 4, "y"); set(g, 10, s.top - 3, "y");
    set(g, 13, s.top - 4, "y"); set(g, 13, s.top - 3, "y");
    set(g, 15, s.top - 4, "z"); set(g, 15, s.top - 3, "y");
  },
  fuzzTop(g, s) { // ふわふわの毛
    [5, 8, 11, 14].forEach((x, i) => {
      const y = s.top - 1 - (i % 2);
      set(g, x, y, "f"); set(g, x, y + 1, "f");
    });
  },
  fuzzRing(g, s, hw) { // まわりのトゲトゲ (フグ・たてがみ)
    const mid = ((s.top + s.bottom) / 2) | 0;
    const rows = [s.top + 2, mid - 2, mid + 3, s.bottom - 3];
    for (const r of rows) {
      const x = GW - (hw[r] || 0) - 2;
      set(g, x, r, "f"); set(g, x + 1, r, "f"); set(g, x, r - 1, "f");
    }
    PARTS.fuzzTop(g, s);
  },
  wingNubs(g, s) { // ちいさなつばさ
    const y = s.eyeY + 8;
    oval(g, 2, y, 1, 3, "v");
    set(g, 3, y - 1, "v"); set(g, 3, y + 1, "v");
  },
  wingBack(g, s) { // おおきなつばさ (背面)
    const y = s.eyeY + 4;
    triUp(g, 2, y + 9, 2, 9, "v");
    set(g, 0, y - 1, "v"); set(g, 1, y - 1, "v");
  },
  butterflyWings(g, s) { // ちょうのはね (背面)
    const t = s.top;
    oval(g, 4, t + 4, 4, 6, "v");
    oval(g, 4, t + 4, 2, 3, "u");
    set(g, 4, t + 3, "y"); set(g, 3, t + 6, "y");
    oval(g, 5, s.bottom - 5, 3, 4, "u");
    set(g, 5, s.bottom - 5, "y");
    set(g, 11, t - 3, "k"); set(g, 10, t - 4, "k"); // しょっかく
  },
  shellBack(g, s) { // こうら (背面)
    const cy = ((s.top + s.bottom) / 2) | 0;
    oval(g, 13, cy, s.width + 1, ((s.bottom - s.top) / 2 | 0) + 1, "s");
    oval(g, 13, cy, s.width - 1, ((s.bottom - s.top) / 2 | 0) - 1, "t");
  },
  moundFront(g, s) { // つちやま (前面)
    const b = s.bottom;
    for (let y = b - 3; y <= Math.min(GH - 2, b + 1); y++) {
      const w = s.width + 1 - Math.max(0, (b - 3 + 2) - y);
  ...
  gem(g, s) { // ひたいのほうせき
    const y = s.top + 3;
    set(g, 15, y, "g"); set(g, 14, y + 1, "g"); set(g, 15, y + 1, "g"); set(g, 15, y + 2, "g");
  },
  moonMark(g, s) { // みかづきもよう
    const y = s.top + 3;
    set(g, 14, y, "y"); set(g, 13, y + 1, "y"); set(g, 13, y + 2, "y"); set(g, 14, y + 3, "y");
  },
  whiskers(g, s) { // ひげ
    const y = s.eyeY + 4;
    fillRect(g, 3, y, 5, y, "k");
    fillRect(g, 3, y + 3, 5, y + 3, "k");
  },
  cheekPads(g, s) { // でんきほっぺ
    oval(g, 7, s.eyeY + 5, 1, 1, "o");
  },
  ...
  legsSide(g, s, hw) { // よこの小さなあし (クモ)
    for (const r of [s.eyeY + 8, s.eyeY + 11, s.eyeY + 14]) {
      const x = GW - (hw[r] || 0) - 1;
      set(g, x, r, "a"); set(g, x - 1, r + 1, "a"); set(g, x, r + 1, "a");
    }
  },
  tuft(g, s) { // ねぐせ毛
    set(g, 15, s.top - 3, "a"); set(g, 15, s.top - 2, "a"); set(g, 14, s.top - 1, "a");
  },
  spikeBack(g, s) { // せなかのトゲ (かいじゅう)
    triUp(g, 5, s.top + 1, 1, 3, "a");
    triUp(g, 10, s.top - 1, 1, 3, "a");
  },
};
...
function specToRows(spec) {
  ...
  const call = (name) => {
    for (const p of parts) {
      ...
      if (n === name) PARTS[n](g, spec, hw);
    }
  };
  const isBack = (n) => ["wingBack", "butterflyWings", "shellBack"].includes(n);
  const isFront = (n) => ["moundFront", "headband", "gem", "moonMark", "whiskers", "cheekPads", "noseBig"].includes(n);

  // 1) 背面パーツ
  for (const p of parts) { const n = Array.isArray(p) ? p[0] : p; if (isBack(n)) PARTS[n](g, spec, hw); }
  // 2) 本体
  ...
  for (const p of parts) {
    const n = Array.isArray(p) ? p[0] : p;
    if (!isBack(n) && !isFront(n)) PARTS[n](g, spec, hw);
  }
  // 4) 自動アウトライン (ふちを k に)
  ...
  // 8) 前面パーツ → 顔
  for (const p of parts) { const n = Array.isArray(p) ? p[0] : p; if (isFront(n)) PARTS[n](g, spec, hw); }
  drawEyes(g, spec);
  drawMouth(g, spec);
  if (spec.blush !== false && spec.mouth !== "beak") {
    const by = spec.eyeY + 5;
    for (const [x, y] of [[7, by], [8, by], [7, by + 1], [8, by + 1]]) {
...
// ---------- 全50体の仕様 ----------
// pal は a(本体) h(明) d(暗) b(おなか) + パーツ色
const SPECS = {
  // --- 御三家 ---
  leafy: { shape: "round", top: 7, bottom: 28, width: 12, eyeY: 13, mouth: "smile",
    parts: ["twinleaf"], pal: { a: "#7ed88a", h: "#b8f0b0", d: "#4aa858", b: "#eafce0", l: "#5cc44e" } },
  foresta: { shape: "square", top: 9, bottom: 29, width: 13, eyeY: 15, mouth: "open", fierce: false,
    parts: ["leafcap"], pal: { a: "#5cb868", h: "#a0e090", d: "#3a8a48", b: "#e0f8d0", l: "#3f9d4c", y: "#f8e04a" } },
  hinokon: { shape: "egg", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "beak",
    parts: ["flame"], pal: { a: "#ffb84d", h: "#ffe0a0", d: "#e08828", b: "#fff0d0", f: "#ff6438", g: "#ffd84d", o: "#ff8c42" } },
  goemba: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["flame"], pal: { a: "#ff7a52", h: "#ffb090", d: "#d84828", b: "#ffd8a8", f: "#ff9a28", g: "#ffe14d" } },
  shizumin: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "open",
    parts: ["droplet", "finGills"], pal: { a: "#7ec4f8", h: "#c0e4ff", d: "#4890d8", b: "#e8f6ff", g: "#a0d8f0" } },
  taidarn: { shape: "round", top: 9, bottom: 28, width: 11, eyeY: 14, mouth: "smile", fierce: true,
    parts: ["shellBack"], pal: { a: "#68aae0", h: "#a8d0f0", d: "#3878b8", b: "#dceeff", s: "#8a6a3d", t: "#d8b878" } },
  // --- 序盤ノーマル/とり/むし ---
  koronezu: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "w",
    parts: ["earRound"], pal: { a: "#e8c088", h: "#f8e0b8", d: "#c09050", b: "#fcf0d8" } },
  dotanezu: { shape: "square", top: 8, bottom: 29, width: 13, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["earRound", "whiskers"], pal: { a: "#c89860", h: "#e0c090", d: "#966838", b: "#f0e0c0" } },
  pipitto: { shape: "egg", top: 9, bottom: 27, width: 10, eyeY: 13, mouth: "beak",
    parts: ["tuft", "wingNubs"], pal: { a: "#ffd866", h: "#fff0b0", d: "#e0a838", b: "#fff8dc", o: "#ff8c42", v: "#f0c050" } },
  sorahane: { shape: "egg", top: 7, bottom: 28, width: 11, eyeY: 12, mouth: "beak", fierce: true,
  ...
  // --- でんき/いわ/エスパー ---
  biribo: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "open",
  ...
  gorotan: { shape: "round", top: 9, bottom: 28, width: 12, eyeY: 14, mouth: "smile",
  ...
  gansekioh: { shape: "square", top: 8, bottom: 29, width: 13, eyeY: 13, mouth: "smile", fierce: true,
    ...
    spots: [[5, 21, "d"], [13, 26, "d"]] },
  nemuriro: { shape: "pear", top: 8, bottom: 28, width: 12, eyeY: 13, eyes: "sleepy", mouth: "w",
    parts: ["tuft"], pal: { a: "#d8bff0", h: "#f0e0ff", d: "#a888cc", b: "#f8f0ff" } },
  yumemira: { shape: "egg", top: 7, bottom: 26, width: 11, eyeY: 12, mouth: "smile", feet: false,
    parts: ["gem"], pal: { a: "#c0a0f0", h: "#e4d4ff", d: "#9068c8", b: "#f0e8ff", g: "#5ce0e8" },
    spots: [[6, 20, "y"], [13, 24, "y"]], extraPal: { y: "#f8e04a" } },
  // --- ゴースト/かくとう ---
  hotabi: { shape: "egg", top: 9, bottom: 28, width: 11, eyeY: 14, mouth: "open", feet: false,
    parts: ["flame"], pal: { a: "#8ed0ff", h: "#d0ecff", d: "#4898e0", b: "#e8f6ff", f: "#68b8ff", g: "#e0f4ff" } },
  onibirasu: { shape: "egg", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "fang", fierce: true, feet: false,
    parts: ["flame", "hornNubs"], pal: { a: "#b8a0f8", h: "#e0d0ff", d: "#8868d0", b: "#eee8ff", f: "#9a7af0", g: "#e8e0ff", y: "#5ce0e8" } },
  kobusshi: { shape: "round", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "smile", fierce: true,
    parts: ["headband"], pal: { a: "#f0c890", h: "#fce4c0", d: "#c89860", b: "#fcf0dc", r: "#f05840" } },
  goukender: { shape: "square", top: 8, bottom: 29, width: 13, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["headband"], pal: { a: "#d8a068", h: "#f0c898", d: "#a87040", b: "#e86848", r: "#c03028" } },
  dokudama: { shape: "pear", top: 9, bottom: 28, width: 13, eyeY: 14, mouth: "open",
    parts: ["drip"], pal: { a: "#c070e0", h: "#e0a8f8", d: "#9040b8", b: "#ecd0f8" },
    spots: [[6, 22, "g"], [12, 25, "g"]], extraPal: { g: "#8ae05a" } },
  ...
  // --- ドラゴン ---
  tatsume: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "open",
  ...
  ryugaoh: { shape: "egg", top: 8, bottom: 29, width: 12, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["hornLong", "wingBack", "spikeBack"], pal: { a: "#6088e0", h: "#98b8f8", d: "#3858b0", b: "#d4e2ff", y: "#f8d060", v: "#4868b8" } },
  tengenryu: { shape: "egg", top: 7, bottom: 28, width: 12, eyeY: 12, mouth: "smile",
    parts: ["antler", "finGills", "gem"], pal: { a: "#8ae8dc", h: "#d0fff6", d: "#4cb8a8", b: "#fbf8ff", g: "#f8d84a" },
    spots: [[6, 21, "y"], [12, 25, "y"]], extraPal: { y: "#f8e04a" } },
  // ============ ここから新モンスター (28-50) ============
  mokoppo: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 14, mouth: "w",
  ...
  hanabii: { shape: "round", top: 10, bottom: 28, width: 11, eyeY: 15, mouth: "smile",
    parts: ["flowerTop"], pal: { a: "#a0d870", h: "#ccf0a0", d: "#70a848", b: "#e8f8d0", y: "#f8d84a" } },
  furawana: { shape: "pear", top: 9, bottom: 28, width: 12, eyeY: 14, mouth: "open",
    parts: ["flowerCrown"], pal: { a: "#88c860", h: "#b8e898", d: "#589838", b: "#f0f8dc", y: "#f8d84a" } },
  kodamakko: { shape: "egg", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "w",
    parts: ["leaf1"], pal: { a: "#d8e8c0", h: "#f0f8e0", d: "#a8c088", b: "#f4fae8", l: "#68b848" },
    spots: [[7, 21, "d"], [12, 24, "d"]] },
  pukupuku: { shape: "round", top: 9, bottom: 27, width: 12, eyeY: 14, mouth: "open",
    parts: ["fuzzRing"], pal: { a: "#90d8e8", h: "#c8f0f8", d: "#58a8c0", b: "#e8f8fc", f: "#f8e070" } },
  harisuin: { shape: "round", top: 8, bottom: 28, width: 13, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["fuzzRing"], pal: { a: "#78b8d8", h: "#b0e0f0", d: "#4888a8", b: "#e0f4f8", f: "#c080e8" } },
  penpeko: { shape: "egg", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "beak",
    parts: ["wingNubs"], pal: { a: "#5878a0", h: "#88a8c8", d: "#385878", b: "#f8fbff", o: "#ffa030", v: "#486890" } },
  denrisu: { shape: "round", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "w",
    parts: ["earCat", "cheekPads"], pal: { a: "#ffe066", h: "#fff4b8", d: "#e0b030", b: "#fffae0", o: "#ff9838" } },
  raikatto: { shape: "egg", top: 7, bottom: 28, width: 11, eyeY: 12, mouth: "fang", fierce: true,
    parts: ["earCat", "cheekPads", "whiskers"], pal: { a: "#ffd028", h: "#fff090", d: "#d8a010", b: "#fff4c8", o: "#ff9838" } },
  pokaguma: { shape: "round", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "w",
    parts: ["earRound"], pal: { a: "#f09858", h: "#ffc898", d: "#c86830", b: "#ffe8cc" } },
  honoguman: { shape: "square", top: 8, bottom: 29, width: 13, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["earRound", "flame"], pal: { a: "#e07038", h: "#ffa070", d: "#b04818", b: "#ffd8a8", f: "#ff9a28", g: "#ffe14d" } },
  tentorin: { shape: "round", top: 9, bottom: 27, width: 11, eyeY: 14, mouth: "smile",
    parts: ["antennaBalls"], belly: false,
    pal: { a: "#f87868", h: "#ffb0a0", d: "#c84838", b: "#ffe8e0", z: "#383848" },
    spots: [[6, 21, "e"], [12, 24, "e"], [9, 26, "e"]] },
  tentogado: { shape: "square", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "smile", fierce: true,
    parts: ["antennaBalls", "rockCrown"], belly: false,
    pal: { a: "#e06858", h: "#f8a090", d: "#b03828", b: "#f8d8d0", z: "#383848" },
    spots: [[6, 21, "s"], [12, 24, "s"]], extraPal: { s: "#9a9a88" } },
  punigumo: { shape: "round", top: 9, bottom: 27, width: 12, eyeY: 14, mouth: "w",
    parts: ["legsSide"], pal: { a: "#b088e0", h: "#d8c0f8", d: "#8058b8", b: "#e8dcf8" },
    extraEyes: true },
  nyaruma: { shape: "round", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "w",
    parts: ["earCat", "whiskers"], pal: { a: "#f8e8d0", h: "#fffaf0", d: "#d0b890", b: "#fffcf4" } },
  nyaruda: { shape: "egg", top: 7, bottom: 28, width: 11, eyeY: 12, mouth: "fang", fierce: true,
    parts: ["earCat", "whiskers"], pal: { a: "#8890a8", h: "#b8c0d0", d: "#586078", b: "#e8ecf4" } },
  fukusuke: { shape: "egg", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "beak",
    parts: ["earCat", "wingNubs"], pal: { a: "#b89068", h: "#d8b890", d: "#886040", b: "#f4ead8", o: "#ffa030", v: "#987850", p: "#d8b890" } },
  panchibi: { shape: "round", top: 8, bottom: 28, width: 11, eyeY: 13, mouth: "smile", fierce: true,
    parts: ["headband", "tuft"], pal: { a: "#f8b070", h: "#ffd8a8", d: "#d08040", b: "#ffeed8", r: "#5878e0" } },
  sheetun: { shape: "egg", top: 8, bottom: 27, width: 11, eyeY: 13, mouth: "open", feet: false,
    parts: ["tuft"], pal: { a: "#f4f4fc", h: "#ffffff", d: "#c8c8dc", b: "#fbfbff" } },
  sunagon: { shape: "square", top: 8, bottom: 28, width: 12, eyeY: 13, mouth: "fang", fierce: true,
    parts: ["hornNubs", "spikeBack"], pal: { a: "#e0c078", h: "#f4e0a8", d: "#b09048", b: "#f8ecc8", y: "#c87840" },
    spots: [[6, 21, "d"], [12, 24, "d"]] },
  yubake: { shape: "round", top: 9, bottom: 28, width: 12, eyeY: 14, eyes: "sleepy", mouth: "open", feet: false,
    parts: ["droplet"], pal: { a: "#88d8d0", h: "#c0f0e8", d: "#50a8a0", b: "#e4f8f4" } },
  tsukiusa: { shape: "egg", top: 9, bottom: 28, width: 11, eyeY: 14, mouth: "w",
    parts: ["earBunny", "moonMark"], pal: { a: "#f0ecff", h: "#ffffff", d: "#c4b8e8", b: "#fbfaff", y: "#f8d84a" } },
};

// クモの追加の目
function addExtraEyes(rows) {
  const g = rows.map((r) => r.split(""));
  set(g, 9, 11, "e"); set(g, 13, 10, "e");
...
for (const [id, spec] of Object.entries(SPECS)) {
  let rows = specToRows(spec);
  if (spec.extraEyes) rows = addExtraEyes(rows);
  MON_SPRITES[id] = { pal: { ...BASE_PAL, ...(spec.extraPal || {}), ...spec.pal }, rows };
}
